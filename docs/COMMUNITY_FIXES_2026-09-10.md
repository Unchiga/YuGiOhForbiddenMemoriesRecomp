...
current StarChips; three totals lines say it in plain words: `SALE VALUE`
(gross), `YOU GET` (the credited amount, with `SELLING n` copies on the right
of that line, or the red `MAX IS 999999` once the limit makes YOU GET smaller
than the sale value), and `YOU WILL HAVE` (the balance after the sale). The
table header is `CARD n OF m / TRUNK / SELL / PRICE` over six rows around the
cursor, with column ticks in the gutters, a gold cursor bar, a gold outline
on the editable SELL cell, and a scrollbar once the trunk holds more than six
cards. Stock-derived prices are gold, card.ini overrides blue, and zero
prices grey. Six-digit prices now end inside the panel (the earlier layout
right-aligned them at x=318 on a 304-pixel canvas and clipped them). A
two-line strip under the table spells out the selected card in full: name,
then `No.NNN`, deck count and trunk copies kept, with a blue `CUSTOM PRICE`
tag only when the price is an override. The second pass (same day) removed
the key-prompt line at the player's request; every binding is unchanged and
only the VIEW / REVIEW / CANCEL buttons are shown. Refusals and notes
(nothing selected, stale inventory or prices, going back from the
confirmation) replace that strip, word-wrapped and centred on its two lines,
until the next press; the old panel never drew them at all.
...
VALUE` list of only the selected cards, one status line (`YOUR DECK IS NOT
CHANGED.`, `THESE CARDS ARE WORTH NOTHING.` or `STARCHIPS STOP AT 999999.`),
and only SELL NOW / GO BACK. Circle there now
...
The 2026-09-11 layout and price-formula work (two passes, the second for
plain wording) was verified on debug binary
`985d5b77ab24e586246609989fe1da9195d1be5eee3788635f4c5e5800f42624` (release
`76eba436bd7d4b8b56d41fc8f18d636381ce8629494ed2ed1af0019817d9b908`). The
regression reran green, 77 of 77, as
`/tmp/ygofm-sell-regression-20260911/results.json` (SHA-256
`2a4d275193b3a90c5db8ceb2579efa7d4c033c91509c82e66abe516cd5fce9ee`; PIDs
3314186/3314465, ports 57729/56515, both exit 0 via `quit_graceful`). The
screen captures come from `tools/sell_ui_capture.py`, whose run
`/tmp/ygofm-sell-ui-20260911/results.json` (SHA-256
`9c7287f2ed7e89e063f62b45443ae131f17686e53412d9b31e18b64d52631fab`; PID
3313507, port 51151, exit 0) passed 25 of 25 checks with controller presses
for every binding. Its `screenshots/` directory holds the composed 960x755
`ui-ui-<state>.png` and the exact native `ui-<state>-native.png` for the
empty trunk, quantities 1/10/99/255, long names, mixed derived/override
prices, zero value, six-digit prices and totals, the 999,999 limit,
confirmation, GO BACK, zero selection, stale transaction, cancellation, a
completed sale and the 722-card scroll. The ydotool keyboard pass proved the
key path on the first-pass binary (X refused with nothing selected, Right
added one, X reviewed, S went back) but did not reach the window on the
second-pass run, which had no keyboard focus; the pad path is unchanged
between the two. `menu_preview --selftest` passes. Known limits: the card
name "Graveyard and the Hand of Invitation" is wider than the 276px detail
line and is cut with `...`; the panel is a host overlay, so the raw
